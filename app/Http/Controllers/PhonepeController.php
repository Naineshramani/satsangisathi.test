    ...
    public function phonepe_redirecturl(Request $request)
    {
        ...
        if ($phonepeVersion == 1) {
            ...
            if ($decoded_data->data->state == 'COMPLETED') {
                if ($payment_type[0] == 'package_payment') {
                    return (new PackagePaymentController)->package_payment_done(Session::get('payment_data'), $moid);
                } elseif ($payment_type[0] == 'wallet_payment') {
                    return (new WalletController)->wallet_payment_done(Session::get('payment_data'), $moid);
                }
            }
        }
