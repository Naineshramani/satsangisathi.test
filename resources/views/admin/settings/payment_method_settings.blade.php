...
@section('content')
    <div class="row">
        ...
        <div class="col-lg-6 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{ translate('UPI Payment') }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('payment_method.update') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input value="1" name="upi_payment_activation" type="checkbox"
                                        @if (get_setting('upi_payment_activation') == 1) checked @endif>
                                    <span class="slider round"></span>
                                </label>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="UPI_ID">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('UPI ID') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="UPI_ID"
                                    value="{{ env('UPI_ID') }}" placeholder="{{ translate('e.g. yourname@bank') }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="UPI_PAYEE_NAME">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('Payee Name') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="UPI_PAYEE_NAME"
                                    value="{{ env('UPI_PAYEE_NAME') }}" placeholder="{{ translate('Name shown to the payer in their UPI app') }}">
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-sm btn-primary">{{ translate('Save') }}</button>
